...
- You can use workspaces to replicate through isolated environments(split state files) as Terraform will create one state file per workspace. So you'll have testing, staging, integration, production workspaces, and so on.  
```terraform workspace new <env> and terraform workspace select <env>.``` 
You will need to create separated .tfvars files, one for each environment.